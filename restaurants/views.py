...
class RestaurantCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
	...
	def post(self, request, *args, **kwargs):
		...
		subject = "A new post has been created in Food Bank"
		msg = "A new post has been created. Do check out by clicking on this link below:\nhttp://" + request.get_host()
		recipient_lst = [email[0] for email in User.objects.filter(is_superuser=False).values_list('email')]
		Thread(target=send_mail, args=(subject, msg, 'ananyaballal4@gmail.com', recipient_lst, True)).start()
		# send_mail(subject, msg, 'pollme700@gmail.com', [], fail_silently=True)
		return resp
